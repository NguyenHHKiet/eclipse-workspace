...
public class Bank {
	...
	public void addTransaction(String custName, Double transaction) {
		int count = 0;
		OUTER_LOOP: for (int i = 0; i < branches.size(); i++) {
			INNER_LOOP: for (int j = 0; j < branches.get(i).getCustomers().size(); i++) {
	...
	public void printCustomerTransactions(String customeName) {
		OUTER_LOOP: for (int i = 0; i < branches.size(); i++) {
			INNER_LOOP: for (int j = 0; j < branches.get(i).getCustomers().size(); j++)
